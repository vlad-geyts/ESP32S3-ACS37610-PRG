...
**Document version:** v2.0 — Incorporates review feedback from `Open_Items_feedback_notes.md` (B1–B6 closed)  
**Status:** Approved
...
The ESP32-S3-DEVKITC-1N16R8V development board is used for this project. It provides native USB-CDC (no external USB-UART bridge needed), the RMT peripheral for precision pulse generation, and sufficient GPIO for power control and status indication. The Controller and Device are connected with a short 50 mm 3-wire cable (VCC, GND, PROG).
...
The CRC is computed MSB first over the fields listed above. The firmware `crc3.c` module must be unit-tested against known-good vectors (synthesised from the polynomial above) before any DUT connection.
...
| DATA[31:26] | Not used (set to 0) | **6-bit Hamming ECC** computed over DATA[25:0] |
...
- **DATA[31:26] must contain valid 6-bit Hamming ECC** computed over DATA[25:0]. The firmware `ecc.c` module implements a standard Hamming(32,26) code. Unit-test with known vectors before DUT use.
- Firmware must **verify after every EEPROM write** by issuing a Read command and comparing the returned value.
...
| GPIO43/44 | UART TX/RX | UART–UART | Low-level debug terminal |
| GPIO19/20 | USB D+/D− | Native USB | USB-CDC host interface (no CH340 required) |
...
| Manchester driver | ESP32-S3 RMT peripheral (TX encoder + RX decoder) |
| ECC module | Software — 6-bit Hamming code over DATA[25:0] for EEPROM writes |
| CRC module | Software — CRC-3, g(x) = x³+x+1, init `0b111` |
| Host interface | USB-CDC (built-in Arduino `Serial` via native USB) |
| Host GUI (optional) | Python + PySerial + Tkinter or Rich CLI |
| Unit tests | Arduino Unity test framework via PlatformIO |

### 4.2 RMT Peripheral Approach (Recommended)

The ESP32-S3 RMT (Remote Control) peripheral generates and captures precise timed pulses in hardware, achieving sub-microsecond accuracy without CPU timing loops or disabled interrupts. Under the Arduino framework (ESP32 Arduino Core), the ESP-IDF RMT driver is accessible directly via the `driver/rmt_tx.h` and `driver/rmt_rx.h` headers.

**TX:** Each Manchester bit is encoded as two RMT symbols of T/2 duration each:
- Bit 0: symbol_low(T/2) + symbol_high(T/2)
- Bit 1: symbol_high(T/2) + symbol_low(T/2)

**RX:** RMT RX channel captures edge timings; the decoder reconstructs bits by comparing each half-period to T/2.

```c
// TX channel setup
rmt_new_tx_channel(&tx_cfg, &tx_chan);
rmt_new_encoder(&encoder_cfg, &manchester_encoder);
rmt_enable(tx_chan);
rmt_transmit(tx_chan, manchester_encoder, frame_bytes, frame_len, &tx_cfg);

// RX channel setup
rmt_new_rx_channel(&rx_cfg, &rx_chan);
rmt_enable(rx_chan);
rmt_receive(rx_chan, rx_buf, rx_buf_size, &rx_cfg);
```

The custom `rmt_encoder_t` implements the Manchester symbol-pair mapping. GPIO direction switching (TX → RX turnaround) is handled in the driver layer between Phase 1 and Phase 2 of a Read operation.

### 4.3 Firmware Module Breakdown

| Module | Responsibility |
|--------|---------------|
| `manchester.c` | RMT TX encoder, RX decoder, GPIO direction control, bit-period configuration |
| `acs37610_cmd.c` | Frame builders for all four commands; CRC computation; ECC computation for EEPROM writes; access-code sequencing; verify-after-write logic |
| `power_ctrl.c` | DUT power-on/off sequencing, settle delays, STATUS_LED state machine |
| `usb_cdc.c` | Arduino `Serial` USB-CDC task; ASCII command parser; response formatter |
| `crc3.c` | CRC-3 module: g(x)=x³+x+1, init=0b111; unit-test vectors derived from polynomial |
| `ecc.c` | 6-bit Hamming ECC computation over DATA[25:0] for EEPROM writes; populates DATA[31:26] |
...
| `READ <addr>` | hex addr | Read register; returns hex data and ECC status |
...
PWRON
  └─► AUTH (Write Access Code: ADDR=0x31, DATA=0x2C413736)
        └─► WEEP <addr> <data>
              ├─► Build frame (compute 6-bit Hamming ECC → DATA[31:26]; compute CRC-3 over R/W+ADDR+DATA)
              ├─► Transmit Manchester frame via RMT TX
...
### Phase 1 — Datasheet Deep-Dive & Protocol Specification (Weeks 1–2)

**Objectives:**
- ~~Obtain CRC polynomial and ECC algorithm~~ — **Closed (B1, B2):** CRC-3 g(x)=x³+x+1 init=0b111; 6-bit Hamming ECC confirmed
...
- Obtain the full ACS37610 programming application note from Allegro (for any remaining undocumented details)
- Confirm PROG pin voltage tolerance (open-drain 3.3 V drive on 3.3 V device — expected compatible)
...
### Phase 2 — Hardware Design & Bring-Up (Weeks 2–4)
...
### Phase 3 — Manchester Driver & RMT Encoder (Weeks 4–6)

**Objectives:**
- Implement RMT TX Manchester encoder under Arduino/PlatformIO (configurable bit period T, default 33 µs / 30 kbps)
- Implement RMT RX Manchester decoder with edge capture
- GPIO direction-switch logic for read turnaround (no extra delay required per B6)
- Implement 74 µs inter-command delay between consecutive R/W operations
- Loopback self-test (TX GPIO → RX GPIO on same DevKit board)
- Verify bit-period accuracy with oscilloscope
- Implement and unit-test CRC-3 module: g(x)=x³+x+1, init=`0b111`, SYNC excluded
- Implement and unit-test 6-bit Hamming ECC module for EEPROM DATA[31:26]
- Unit-test all four command frame builders with known register values

**Deliverables:** Passing PlatformIO unit-test suite; oscilloscope captures showing correct Manchester waveform at 30 kbps.
...
- Handle and report all error conditions: CRC mismatch, ECC fail, verify fail, timeout, WRITE_LOCK set
...
|-------|-------|----------|
| 1 — Protocol spec | 1–2 | 2 weeks |
| 2 — Hardware | 2–4 | 2 weeks |
| 3 — Manchester driver | 4–6 | 2 weeks |
| 4 — DUT integration | 6–9 | 3 weeks |
| 5 — GUI & validation | 9–12 | 3 weeks |
| **Total** | **1–12** | **12 weeks** |
...
**Severity:** Low *(downgraded from High — CRC, ECC, register map, and access code are now confirmed)*  
**Probability:** Low  
**Mitigation:** CRC-3 polynomial, 6-bit Hamming ECC, full register map, and access code are documented in §2 (B1–B4 closed). Obtain the full Allegro programming application note to catch any remaining edge cases (preamble requirements, error recovery). Implement configurable T in firmware.
...
### R3 — CRC or ECC implementation error in firmware
**Severity:** High  
**Probability:** Low *(algorithm confirmed — risk is now implementation, not specification)*  
**Mitigation:** CRC-3 (g(x)=x³+x+1, init=0b111) and 6-bit Hamming ECC are confirmed (B1, B2). Generate unit-test vectors analytically from the polynomial before coding. Unit-test both modules in PlatformIO before any DUT connection. Begin Phase 4 with READ-only operations to validate frame structure independently of write logic.

---

### R4 — RMT timing jitter exceeds device tolerance
**Severity:** Medium  
**Probability:** Low  
**Mitigation:** Under the Arduino framework, pin the programmer task to Core 1 using `xTaskCreatePinnedToCore()` at high priority to avoid Wi-Fi/BLE scheduler interference. Validate bit-period accuracy with oscilloscope before connecting to a real device. Use the conservative default bit rate (30 kbps / T=33 µs) rather than the 333 kbps maximum.
...
**Severity:** Low  
**Probability:** Low  
**Mitigation:** The Arduino framework on ESP32-S3 exposes native USB-CDC as the standard `Serial` object. The DevKit board uses the built-in USB port directly. Test enumeration on Windows 10/11, macOS, and Ubuntu. Install the Espressif USB CDC driver if required on older Windows systems.
...
1. **Set up VS Code + PlatformIO** with the ESP32 Arduino Core. Verify the ESP32-S3-DEVKITC-1N16R8V enumerates as a USB-CDC serial device on the development PC.

2. **Receive and inspect the ACS37610 custom eval board.** Verify supply voltage (3.3 V), check PROG pin accessibility, and confirm connector pinout before wiring.

3. **Build and test the PROG line interface.** Wire the 10 kΩ pull-up and TVS diode on the breadboard. Verify the open-drain GPIO on the DevKit can drive the line low and release it cleanly before connecting to the DUT.
...
| B2 | ECC algorithm for DATA[31:26] | ✅ Closed | 6-bit Hamming code over DATA[25:0] — see §2.5.2 |
...
| B5 | Hardware, Firmware, BOM, Risk sections | ✅ Closed | H/W: DevKit + custom eval board + 3-wire cable. Firmware: VS Code + PlatformIO + Arduino. BOM and Risk: confirmed |
| B6 | PROG pin turnaround timing | ✅ Closed | No turnaround delay required; 74 µs inter-command delay confirmed — see §2.2 |

---

*End of document — ACS37610 Programmer Development Plan v2.0*
