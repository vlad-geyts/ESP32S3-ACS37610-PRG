...
**Document version:** v3.0 — Incorporates Phase 3 pre-implementation review: ECC clarification (device generates ECC automatically), module files changed to `.cpp`, native PlatformIO test environment added, RMT open-drain and RX idle threshold notes added  
**Status:** Approved
...
**Open-drain requirement:** GPIO4 must have open-drain mode explicitly enabled before the RMT TX channel is attached. RMT driving HIGH on an open-drain GPIO releases the line; the pull-up provides the high state. RMT driving LOW actively sinks current.

```cpp
gpio_set_drive_capability(GPIO_NUM_4, GPIO_DRIVE_CAP_0);
gpio_od_enable(GPIO_NUM_4);  // must be called before rmt_new_tx_channel()
```

**TX → RX turnaround (Read Phase 2):** No mandatory delay is required between phases, but the GPIO switch must complete before the device begins its response frame:
1. Wait for RMT TX done callback (frame fully clocked out)
2. Detach GPIO4 from RMT TX channel; configure as input (releases open-drain line)
3. Attach GPIO4 to RMT RX channel and arm receive
4. Device drives SYNC + DATA + CRC response; RMT RX captures

**RX idle threshold:** Configure to ~50 µs at 30 kbps (≈ 1.5 × T/2 = 25 µs) to detect end-of-frame reliably. The RX filter threshold should be ~4 µs to reject sub-half-period glitches while passing valid T/2 = 16.5 µs edges.

The custom `rmt_encoder_t` implements the Manchester symbol-pair mapping. GPIO direction switching is handled in the driver layer between Phase 1 and Phase 2 of a Read operation.

### 4.3 Firmware Module Breakdown

| Module | Responsibility |
|--------|---------------|
| `manchester.cpp` | RMT TX encoder, RX decoder, GPIO direction control, open-drain configuration, bit-period configuration |
...
**Objectives:**
- Implement `crc3.cpp` module and unit-test via `[env:native]` (runs on PC, no hardware) — **start here**
- Add `[env:native]` to `platformio.ini` for PC-hosted unit tests
- Implement RMT TX Manchester encoder under Arduino/PlatformIO (configurable bit period T, default 33 µs / 30 kbps)
- Configure GPIO4 open-drain mode (`gpio_od_enable`) before RMT TX channel attachment
- Implement RMT RX Manchester decoder with edge capture; set idle threshold ~50 µs, filter ~4 µs
- GPIO direction-switch logic for TX → RX turnaround (no extra delay required per B6)
...
- Verify bit-period accuracy with oscilloscope using GPIO21 (STROB_OUT) as scope trigger
- Unit-test all four command frame builders with known register values

**Deliverables:** Passing PlatformIO unit-test suite (`[env:native]` CRC-3 tests + `[env:esp32-s3-devkitc-1-n16r8v]` loopback); oscilloscope captures showing correct Manchester waveform at 30 kbps.
...
| 3 — Manchester driver | 4–6 | 2 weeks | In progress |
...
**Severity:** Medium  
**Probability:** Low  
**Mitigation:** Pin the programmer task to Core 1 using `xTaskCreatePinnedToCore()` at high priority to avoid Wi-Fi/BLE scheduler interference. Validate bit-period accuracy with oscilloscope (using GPIO21 STROB_OUT as trigger) before connecting to a real device. Use the conservative default bit rate (30 kbps / T=33 µs) rather than the 333 kbps maximum.
...
*End of document — ACS37610 Programmer Development Plan v3.0*
