...
**Document version:** v4.1 — Added communication port open requirement: the ACS37610 serial port must be opened by sending the Access Code before any Read or Write command (not only before EEPROM writes). Post-Access-Code settle time of 90–120 µs documented. §2.2, §2.4, §4.4, §4.5 updated accordingly.  
*(v4.0: TX architecture change: Manchester TX implemented as bit-bang. RMT TX abandoned. PROG idle voltage root cause documented. Phase 3 TX validation complete.)*  
**Status:** Approved
...
| Post-Access-Code settle time before first R/W command | 90 | — | 120 | µs |
| EEPROM write time t_w | — | 25 | 35 | ms |

**Notes:**
- **Communication port must be opened before any Read or Write command.** Send the Access Code (§2.4) immediately after power-on. The port remains open until the device is power-cycled. The first Read or Write command must not be issued until 90–120 µs after the Access Code frame completes.
- No special timing or voltage requirements for read operations once the port is open.
...
Opens the serial communication port on the ACS37610. **This command must be sent before any Read or Write command** — the device will not respond to Read or Write frames until the port has been opened. The access code is fixed for the ACS37610 family.
...
**Behaviour:**
- **Opens the serial communication port.** Until this command is accepted, the device does not respond to any Read or Write command.
- The port remains open until the device is power-cycled (reset). It does not need to be re-sent between consecutive Read/Write operations within the same power session.
- This command must be re-sent after every power cycle before issuing any Read or Write command.
- **Post-Access-Code settle time:** Wait 90–120 µs after the Access Code frame ends before issuing the first Read or Write command.
- In addition to opening the port, this command also unlocks EEPROM write operations (required before any `WEEP` command).
...
|---------|-----------|--------|
| `AUTH` | — | Send Write Access Code frame (`ADDR=0x31`, `DATA=0x2C413736`); opens serial communication port; waits 120 µs settle before returning. **Required after every power cycle before any READ or WRITE command.** |
...
### 4.5 Communication Port Opening & Command Sequences (Firmware State Machine)

The Access Code must be sent immediately after power-on, before any Read or Write command. The port remains open for the entire power session.

#### 4.5.1 Power-On Sequence (mandatory for all operations)

```
PWRON
  └─► AUTH (Write Access Code: ADDR=0x31, DATA=0x2C413736)
        └─► Wait 120 µs (post-Access-Code settle — port now open)
              └─► [ready for READ or WRITE commands]
```
...
[port open]
  └─► READ <addr>
        ├─► Build 12-bit read request frame (CRC-3 over R/W+ADDR)
        ├─► Transmit via bit-bang TX; release PROG line
        ├─► ARM RMT RX; wait up to 100 ms for 44-bit device response
        └─► Decode SYNC + DATA[31:0] + CRC; check DATA[27:26] ECC status (EEPROM reads)
              ├─► Valid + ECC OK → return DATA[25:0]
              ├─► CRC mismatch   → return ERR_CRC
              └─► ECC fault      → return ERR_ECC
```

#### 4.5.3 EEPROM Write Sequence

```
[port open — AUTH already sent this session]
  └─► WEEP <addr> <data>
...
**Objectives:**
- Connect real ACS37610 eval board via 50 mm 3-wire cable; power on, send AUTH (Access Code), wait 120 µs settle, then begin with READ of FAULT_STATUS (`0x20`) — non-destructive diagnostic. Verify device responds (no response before AUTH confirms port-open requirement)
...
- Implement and test full power-on sequence: AUTH (`ADDR=0x31`, `DATA=0x2C413736`) → 120 µs settle → READ / WEEP → verify
...
*End of document — ACS37610 Programmer Development Plan v4.1*
