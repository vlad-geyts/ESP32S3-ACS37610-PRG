...
// Expand RMT items to a half-period stream, then decode Manchester bit pairs.
// Returns bit count on success, 0 on any format error.
static uint8_t decode_rmt(const rmt_item32_t *items, size_t count,
                           uint32_t half_ticks, uint64_t *out) {
    // A pulse whose duration >= 1.5 × half_ticks spans two half-periods (T duration).
    ...
    for (size_t i = 0; i < count && n < 88u; ++i) {
        // First pulse in item
        if (items[i].duration0 && n < 88u) {
            halves[n++] = items[i].level0;
            if (items[i].duration0 >= thresh && n < 88u) halves[n++] = items[i].level0;
        }
        // Second pulse in item (duration1 == 0 marks the end-of-data terminator)
        if (items[i].duration1 && n < 88u) {
            halves[n++] = items[i].level1;
            if (items[i].duration1 >= thresh && n < 88u) halves[n++] = items[i].level1;
        }
    }
